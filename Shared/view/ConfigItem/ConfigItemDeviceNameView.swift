//--------------------------------------------------------------------------------//
// @file  ConfigItemDeviceNameView.swift
// @version $Revision$
...
struct ConfigItemDeviceNameView : View
{
  ...
  // Title
  //--------------------------------------------------------------------------------------------------------//
  @State var strTitle        : String   = "Device Name"
  @State var strDescription  : String   = "Device Name for BLE discovering"

  //--------------------------------------------------------------------------------------------------------//
  // Button
  //--------------------------------------------------------------------------------------------------------//
  @State var strRead         : String   = "Read"
  @State var strWrite        : String   = "Write"
  ...
  // SREG_NAME
  //--------------------------------------------------------------------------------------------------------//
  @State var strSRegName  : String = SREG_DEVICE_NAME
  ...
  func onBtnWriteClicked()
  {
    ...
    {
      self.strAlertInfo = "Write value cannot be empty"
    ...
    if(bRet == FALSE)
    {
      ...
      self.strAlertInfo = "Command is busy"
      self.bAlertShow   = TRUE
      return
    }
    else
  ...
  func UIUpdate()
  {
    ...
    //----------------------------------------------------------------//
    self.strTitle         = "Device name"
    self.strDescription   = "Device name for BLE scanning display"
    self.strWriteNote     = "Enter here"
    self.strRead          = "Read"
    self.strWrite         = "Write"
  }
  ...
  var body: some View
  {
    ...
    ZStack
    {
      ...
      RoundedRectangle(cornerRadius: 25)
        ...
        .frame(width: self.fViewWidth, height: 240, alignment: .leading)
      ...
      VStack
      {
        ...
        HStack
        {
          ...
          {
            Rectangle()
              .stroke(self.colorFieldBorder)
              .frame(width: self.fFieldWidth, height: 32, alignment: .leading)
            Text(self.strReadValue)
              .padding()
            ...
            .frame(width: self.fFieldWidth, height: self.fFieldHeight, alignment: .leading)
          Spacer()
            .frame(width:20)
          
          Button(action: self.onBtnReadClicked)
          {
            Text(self.strRead)
            ...
            .background(self.colorBtnReadBg)
        ...
        HStack
        {
          ...
          {
            Rectangle()
              .fill(self.colorFieldBgColor)
              .frame(width: self.fFieldWidth, height: 32, alignment: .leading)
            TextField(self.strWriteNote, text: $strWriteValue)
              .frame(width: self.fFieldWidth, height: self.fFieldHeight, alignment: .center)
          }
            .frame(width: self.fFieldWidth, height: self.fFieldHeight, alignment: .leading)
          Spacer()
            .frame(width:20)
      ...
      .frame(width: self.fViewWidth, height: 240, alignment: .center)
